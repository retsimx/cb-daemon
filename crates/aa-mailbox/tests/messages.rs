...
fn golden_write_full_addressing() {
    ...
    assert_eq!(v["unit_id"], "181f3");
    assert_eq!(v["zone"], 1);
    let expected: Value = serde_json::from_str(
        r#"{"type":"write","msg_id":"req-1","unit_type":"07","unit_id":"181f3","register":"03","zone":1,"payload":{"open":true,"damper_pct":100,"sensor_type":"wired","target_temp_c":24.0,"measured_temp_c":23.1}}"#,
    )
    .unwrap();
    assert_eq!(v, expected);
    assert_eq!(
...
fn golden_snapshot_message() {
    ...
    let msg = ServerMessage::Snapshot {
        units: BTreeMap::from([("07:181f3".to_owned(), registers)]),
    };
    let v = serde_json::to_value(&msg).unwrap();
    assert_eq!(v["type"], "snapshot");
    let expected: Value = serde_json::from_str(
        r#"{"type":"snapshot","units":{"07:181f3":{"01":{"header":32,"total_zones":3,"constant_zones":1,"constant_zone_ids":[1,0,0],"filter_clean_required":false}}}}"#,
    )
    .unwrap();
    assert_eq!(v, expected);
    assert_eq!(
...
fn golden_snapshot_nested_zones_and_raw_hex() {
    ...
    let v = serde_json::to_value(&msg).unwrap();
    assert_eq!(v["units"]["07:181f3"]["03"]["1"]["damper_pct"], 100);
    assert_eq!(v["units"]["07:181f3"]["16"], "a1b2c3d4e5f607");
...
fn golden_event_message() {
    ...
    assert!(v.get("zone").is_none());
    let expected: Value = serde_json::from_str(
        r#"{"type":"event","unit_type":"07","unit_id":"181f3","register":"05","payload":{"power":"on","mode":"cool","fan":"high","target_temp_c":24.0,"myzone_id":0,"fresh_air":false,"rf_sys_id":0}}"#,
    )
    .unwrap();
    assert_eq!(v, expected);
    assert_eq!(
...
fn golden_event_message_with_zone() {
    ...
    assert_eq!(v["zone"], 2);
    let expected: Value = serde_json::from_str(
        r#"{"type":"event","unit_type":"07","unit_id":"181f3","register":"04","zone":2,"payload":{"min_damper":10,"max_damper":90,"motion_status":1,"motion_config":2,"zone_error":0,"rssi":45}}"#,
    )
    .unwrap();
    assert_eq!(v, expected);
    assert_eq!(
...
fn golden_read_result_message() {
    ...
    assert_eq!(v["zone"], 1);
    let expected: Value = serde_json::from_str(
        r#"{"type":"read_result","msg_id":"req-4","unit_type":"07","unit_id":"181f3","register":"03","zone":1,"payload":{"open":true,"damper_pct":100,"sensor_type":"wired","target_temp_c":24.0,"measured_temp_c":23.1}}"#,
    )
    .unwrap();
    assert_eq!(v, expected);
    assert_eq!(
...
fn snapshot_units_covers_all_unit_types() {
    ...
    bank.apply(&CanRecord::parse_one("0703181f30501010330000100").unwrap());
    bank.apply(&CanRecord::parse_one("0803181f30501010330000100").unwrap());
    bank.apply(&CanRecord::parse_one("0803abcde0601020300000000").unwrap());

    let units = snapshot_units(&bank);
    assert_eq!(
        units.keys().collect::<Vec<_>>(),
        vec!["07:181f3", "08:181f3", "08:abcde"]
    );
}
...
fn snapshot_units_decodes_dtos_and_nested_zones() {
    ...
    bank.apply(&CanRecord::parse_one("0703181f30501010330000100").unwrap());
    bank.apply(&CanRecord::parse_one("0703181f30301e40230170100").unwrap());

    let units = snapshot_units(&bank);
    let unit = &units["07:181f3"];
    assert_eq!(
        unit["05"],
        json!({
            "power": "on",
...
fn event_body_zone_bearing_and_plain_records() {
    ...
    let record = CanRecord::parse_one("0703181f30301e40030000000").unwrap();
    ...
    let record = CanRecord::parse_one("0703181f30501010330000100").unwrap();
    let (register, zone, payload) = event_body(&record);
    assert_eq!(register, "05");
    assert_eq!(zone, None);
    ...
    let record = CanRecord::parse_one("0703181f316a1b2c3d4e5f607").unwrap();
    let (register, zone, payload) = event_body(&record);
