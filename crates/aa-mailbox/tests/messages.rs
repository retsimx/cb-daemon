...
fn golden_snapshot_message() {
    ...
    let expected: Value = serde_json::from_str(
        r#"{"type":"snapshot","units":[{"unit_type":"07","unit_id":"181f3","registers":{"01":{"header":32,"total_zones":3,"constant_zones":1,"constant_zone_ids":[1,0,0],"filter_clean_required":false}}}]}"#,
    )
    .unwrap();
    assert_eq!(v, expected);
    assert_eq!(
